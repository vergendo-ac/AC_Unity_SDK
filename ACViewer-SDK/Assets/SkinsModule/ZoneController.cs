...
public class ZoneController : MonoBehaviour
{
    ...
    public ServerURLs URLs; // URLSData.Instance?.URLs
    ...
    void Start()
    {
        ...
        // load UI contrller for app params
    ...
    void AppParamsParser(string jsonS)
    {
        ...
        do
        {
            ...
            {
                PlaceInfo newPlace = new PlaceInfo((pl != null ? pl : ""), la, lo, ra);
			    ...
			    Debug.Log($"AppParamsParser: zone[{count}] = {newPlace.placeName}");
            }
			else break;
        } while (true);
    ...
    public void DoCheckLocation(PlaceInfo[] places)
    {
        ...
        for (int i = 0; i < places.Length; i++) 
        {
            ...
            {
                oPlace = places[i];
                Debug.Log($"DoCheckLocation: oPlace={oPlace.placeName},[{i}]");
            }
            else
            {
                float la = 0;
                ...
                {
                    rPlace = places[i];
                    Debug.Log($"DoCheckLocation: rPlace=<{rPlace.placeName}>,[{i}]");
                }
            }
        }
        ...
        {
            PlaceInfo defParams = new PlaceInfo("def");
    ...
    void SetNotInPlace()
    {
        Debug.Log("DoCheckLocation: NotInPlace=1");
    ...
    // It's connected with the Repeat() button of the main panel
